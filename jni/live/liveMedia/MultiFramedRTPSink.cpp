...
/*
isFirstPacket = true 传输开始后的第一个包 continuePlaying
isFirstPacket = false 传输开始后第一个包以后(不是帧的第一个包) sendNext 
...
void MultiFramedRTPSink::buildAndSendPacket(Boolean isFirstPacket) {
  ...
  // RTP header:
  fSpecialHeaderPosition = fOutBuf->curPacketSize(); // 记录头部的位置

  // JPEGVideoRTPSink::specialHeaderSize 会根据是否第一个包来判断头部是否加入量化表
  fSpecialHeaderSize = specialHeaderSize(); // 获得H264/RTP JPEG/RTP头部长度
  fOutBuf->skipBytes(fSpecialHeaderSize); // 跳过头部
...
::afterGettingFrame1(unsigned frameSize, unsigned numTruncatedBytes,
  ...
  if (numFrameBytesToUse == 0 && frameSize > 0) {
    ...
    // 一个包即将发送 这里再获取协议头部 JPEG/RTP H264/RTP
    // 对于JPEG/RTP来说(JPEGVideoRTPSink), 会判断curFragmentationOffset是否第一个RTP包 然后再打包量化表进去
...
void MultiFramedRTPSink::sendPacketIfNecessary() {
  ...
  if (fNoFramesLeft) {
    ...
    gettimeofday(&timeNow, NULL); // ?? 可以设置fNextSendTime来决定后面包的发送时间??
	...
	fprintf(stderr, " sendNext package! ! %d us\n" , uSecondsToGo);
	
    // Delay this amount of time: 延时uSecondsToGo之后再执行sendNext发送下一个包
    void* next = envir().taskScheduler().scheduleDelayedTask(uSecondsToGo, (TaskFunc*)sendNext, this);
	fprintf( stderr , "sendNext %p schedule ", next );
    nextTask() = next ;	
  }
}
...
void MultiFramedRTPSink::sendNext(void* firstArg) {
	...
	fprintf( stderr , "sendNext entry ", __LINE__ );

  MultiFramedRTPSink* sink = (MultiFramedRTPSink*)firstArg;
  sink->buildAndSendPacket(False); // False = 不是开始传输的第一个包
}
