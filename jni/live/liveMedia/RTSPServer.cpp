...
RTSPServer::createNew(UsageEnvironment& env, Port ourPort,
  ...
  if (ourSocket == -1) return NULL;
  	...
  	// 在GenericMediaServer注册listen回调函数
...
::unnoteTCPStreamingOnSocket(int socketNum, RTSPClientSession* clientSession, unsigned trackNum) {
	...
	fprintf(stderr , "unnoteTCPStreamingOnSocket = %d",socketNum );
...
void RTSPServer::stopTCPStreamingOnSocket(int socketNum) {
	fprintf(stderr , "stopTCPStreamingOnSocket = %d",socketNum );
...
RTSPServer::RTSPClientConnection::~RTSPClientConnection() {
	fprintf(stderr , "~RTSPClientConnection");
    ...
    */
	...
	if (strcmp(cmdName, "GET_PARAMETER") == 0) {
	  ...
	  handleCmd_notSupported();
      	...
      	// 这里找到 rtspServer 中对应uri的 ServerMediaSession实例  返回流媒体信息SDP
	handleCmd_DESCRIBE(urlPreSuffix, urlSuffix, (char const*)fRequestBuffer);
      } else if (strcmp(cmdName, "SETUP") == 0) {
	Boolean areAuthenticated = True;

	if (!requestIncludedSessionId) { // ?? 也就是SETUP带 track=0 track=1
	  ...
	  if (authenticationOK("SETUP", urlTotalSuffix, (char const*)fRequestBuffer)) {
	    clientSession // 如果SETUP track=0 track=1就有两个 RTSPClientSession
	      = (RTSPServer::RTSPClientSession*)fOurRTSPServer.createNewClientSessionWithId();
	  } else {
	    areAuthenticated = False;
	  }
	}
	if (clientSession != NULL) {// 创建RTSPClientSession之后 调用其handleCmd_SETUP
...
#endif
	handleCmd_bad(); // 什么请求都不是 返回400 Bad Request
      }
    }
...
fprintf(stderr , "reclaimStreamStates");
  for (unsigned i = 0; i < fNumStreamStates; ++i) {
    if (fStreamStates[i].subsession != NULL) {
...
/* 检查 Transport: 字段
	...
	< Transport: RTP/AVP/TCP;unicast;destination=192.168.43.133;source=192.168.43.1;interleaved=0-1


	> Transport: RTP/AVP;unicast;client_port=54348-54349
	< Transport: RTP/AVP;unicast;destination=192.168.43.133;source=192.168.43.1;client_port=54348-54349;server_port=6970-6971

	如果不指定为 RTP/AVP/TCP 或者 RAW/RAW/UDP的话 都作为 RTP/AVP(UDP)

	如果用udp的话 会看到
	netstat 会看到 6970 - 6971
	udp    6016  99072 0.0.0.0:6970 	  0.0.0.0:*		 CLOSE
	udp	  0	 0 0.0.0.0:6971 	  0.0.0.0:*		 CLOSE


	客户端通过(见testRTSPClient.cpp)
	...
	unsigned RTSPClient::sendSetupCommand(MediaSubsession& subsession, responseHandler* responseHandler,
				      ...
				      streamUsingTCP = true 来指定 RTP over TCP/UDP
*/
static void parseTransportHeader(char const* buf,
  ...
  while (sscanf(fields, "%[^;\r\n]", field) == 1) {
    if (strcmp(field, "RTP/AVP/TCP") == 0) {
      ...
      // 如果传输模式是RAW_UDP的话 rtcp端口设置为0 其他的话 默认是1 或者客户指定
...
// 从客户端的请求中获取一些客户端参数 e.g 客户端的RTP端口、RTCP端口
// 
...
::handleCmd_SETUP(RTSPServer::RTSPClientConnection* ourClientConnection,
  ...
  do {
	...
	*/
    ...
    if (fIsMulticast) { // multicast 组播方式 只支持 RTP_UDP / RAW_UDP 
    ...
    } else { // unicast 单播方式  支持 RTP_UDP RTP_TCP RAW_UDP
      switch (streamingMode) {
          case RTP_UDP: {
	    snprintf((char*)ourClientConnection->fResponseBuffer, sizeof ourClientConnection->fResponseBuffer,
		     "RTSP/1.0 200 OK\r\n"
...
RTSPServer::createNewClientConnection(int clientSocket, struct sockaddr_in clientAddr) {
  // 如果客户端连接上  accept 之后 用 RTSPClientConnection 对应之
