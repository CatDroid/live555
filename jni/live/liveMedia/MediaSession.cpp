...
Boolean MediaSession::initializeWithSDP(char const* sdpDescription) {
  ...
  while (sdpLine != NULL) {
	...
	// payloadFormat   m=video 0 		RTP/AVP 26   MJPEG
	//		   m=video 5006 	RTP/AVP 96   H.264
	...
	if (sdpLine[0] == 'm') break; // we've reached the next subsession 下一个m行subsession
      }
      continue;
    }
    ...
    subsession->fMediumName = strDup(mediumName); // video or audio 
    delete[] mediumName;
    subsession->fProtocolName = strDup(protocolName); 	// RTP/AVP  RTP/AVP 	=> RTP
    													// UDP udp RAW/RAW/UDP => UDP
    subsession->fRTPPayloadFormat = payloadFormat;

    // Process the following SDP lines, up until the next "m=":
    while (1) { // 对应这个m=也就是subsession子会话的其他参数
      sdpLine = nextSDPLine;
      if (sdpLine == NULL) break; // we've reached the end
      if (!parseSDPLine(sdpLine, nextSDPLine)) return False;
...
Boolean MediaSubsession::initiate(int useSpecialRTPoffset) {
  ...
  do {
    ...
    if (fClientPortNum != 0 && (honorSDPPortChoice || IsMulticastAddress(tempAddr.s_addr))) {
      ...
      // 创建RTP socket
      if (protocolIsRTP && !fMultiplexRTCPWithRTP) { // 如果使用RTP 但是RTP和RTCP使用不同的端口
	fClientPortNum = fClientPortNum&~1; // RTP端口一定是偶数 
	...
	if (fMultiplexRTCPWithRTP) {
      ...
      // 如果端口没有预先指定 那么我们使用临时的端口
      // 如果RTCP复用RTP 创建一个socket
	...
	if (isSSM()) {
	  ...
	  fRTPSocket = new Groupsock(env(), tempAddr, 0, 255); // 不指定端口
	}
	if (fRTPSocket == NULL) {
	...
	// Get the client port number:  获得RTP socket的端口号
	Port clientPort(0);
